...
Explicit `-a`/`--agent` always overrides both, and `-r`/`--resume` bypasses both so the thread's original agent is restored. See [Command reference](/oss/python/deepagents/cli/overview#command-reference) for related flags.
...
<ResponseField name="DEEPAGENTS_CLI_LANGSMITH_PROJECT" type="string" post={["optional"]}>
    Override the LangSmith project name for agent traces. See [Trace with LangSmith](/oss/python/deepagents/cli/overview#trace-with-langsmith).
</ResponseField>
