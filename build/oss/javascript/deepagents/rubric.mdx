...
To carry rubrics over to follow up invocations, attach a [checkpointer](/oss/javascript/langgraph/persistence#checkpoints) and pass the same `thread_id` alongside the invocation. In these cases, the same `rubric` persists across future `invoke` calls until the user passes a new one in.
