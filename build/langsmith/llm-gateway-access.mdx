...
### Default: the gateway project

By default, all gateway-proxied calls are traced to a project named `gateway` in the [workspace](/langsmith/administration-overview#workspaces) associated with the caller's API key. This project is accessible to [Organization admins](/langsmith/rbac/#organization-admin) with gateway edit permissions.

### Project routing

Gateway-proxied LLM calls always land in a fixed `gateway` project. The gateway sets the `Langsmith-Project` header itself on the outbound OTLP upload. It does **not** read a `Langsmith-Project` header (or SDK-set project) from the caller, so traces cannot be routed into a caller's own project (for example, `my-agent`). For per-API-key isolation in the UI, each trace is also copied to a second project named `gateway-<short_api_key>-<api_key_id>`. Gateway-specific metadata is attached to the spans in both copies.

Set [ABAC](/langsmith/abac) policies against these individual projects to limit view access to only the intended audience.
...
| **Gateway invocations** | Each proxied call, including the caller identity, provider, model, and policy evaluation result. |
...
Always use workspace-scoped API keys for the gateway. Organization-scoped keys are not recommended because the gateway's permission checks and trace routing are both workspace-scoped. A workspace-scoped key gives you clean policy matching and ensures traces land in the right project.

Each API key can carry its own spend limit and policy bindings to scope agents independently.
...
- **Project-level access policies** (requires an [Enterprise plan](/langsmith/pricing-plans)): write a policy restricting `projects:read` and `runs:read` on the gateway project to specific users or roles.
