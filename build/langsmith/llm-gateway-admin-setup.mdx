...
At General Access, this will be a self-service setting.
...
Policies are optional during initial setup. The gateway functions as a transparent proxy until you have configured policies.
...
<Warning>
Use workspace-scoped keys, not organization-scoped keys. The gateway's permission checks and trace routing are both workspace-scoped. Org-scoped keys are not recommended.
</Warning>
...
Ask a user to run the verification curl from the [Quickstart](/langsmith/llm-gateway-quickstart):
...
A `200` response confirms the gateway, the API key, provider secrets, and role permissions are all configured correctly. The call will appear as a trace in the **gateway** tracing project in the workspace.
