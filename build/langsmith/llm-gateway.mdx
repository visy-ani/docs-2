...
| **Spend visibility** | Real-time cost rollups by workspace, user, API key, and model. |
| **PII redaction** | Detects and redacts names, nationality, locations, US SSNs, and US phone numbers from requests before they reach the model. |
| **Secrets redaction** | Detects and redacts API keys, tokens, and credentials from requests. Covers AWS, GitHub, GitLab, OpenAI, Anthropic, GCP, Azure, Slack, Datadog, PyPI, npm, private keys, and LangSmith tokens. |
...
| **Trace continuity** | Gateway calls appear in the caller's tracing project alongside their other traces. |
...
- **Claude Desktop:** no marketplace plugins supported. Chat on Desktop is not supported.
- **Codex Desktop:** no marketplace plugins supported.
- **Cursor:** IDE integration is not yet available. Only OpenAI base URL configuration is supported.
