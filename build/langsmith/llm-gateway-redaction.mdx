...
| **Nationality, religion, or politics** | Nationality |
...
Detection uses Presidio for named entities (names, locations, demographic attributes) and pattern-based rules for structured identifiers (SSNs, phone numbers).
...
- **Scanner failures fail-open:** if a PII or secrets scanner is unreachable, slow, or errors, that stage is skipped and the request passes through unredacted.
